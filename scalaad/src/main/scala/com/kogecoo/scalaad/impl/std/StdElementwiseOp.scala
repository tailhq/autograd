...
object StdElementwiseOp {
  ...
  def nullary(n: V): () => T0 = n match {
    case Zero(_) => () => 0.0
    case Half(_) => () => 0.5
    case One (_) => () => 1.0
  }

  def unary(n: V): (T0 => T0) = n match {
    case _: Pos      => +_
    case _: Neg      => -_
    case _: Identity => v => v
  ...
  def binary(n: V): (T0, T0) => T0 = n match {
    case _: Add => (x, y) => x + y
    case _: Sub => (x, y) => x - y
    case _: Mul => (x, y) => x * y
    case _: Div => (x, y) => x / y
...
object StdElementwiseOpB {
  ...
  def unary(n: B): (B0 => B0) = n match {
    case _: Not => v => !v
  }

  def binary(n: B): ((B0, B0) => B0) = n match {
    case _: And => (x, y) => x && y
    case _: Or  => (x, y) => x || y
  }
...
object StdElementwiseOpC {
  ...
  def binary(n: B): ((T0, T0) => B0) = n match {
    case _: Eq  => (x, y) => x == y
    case _: Neq => (x, y) => x != y
    case _: Lt  => (x, y) => x < y
    case _: Lte => (x, y) => x <= y
