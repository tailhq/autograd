...
import com.kogecoo.scalaad.graph.{Acos1, N1, N2, Var1}
...
object StdAcos1Spec extends Properties("Acos1") with Acos1Spec with StdSpecBackend {
  ...
  override def expectApplyOp(a: N1): T1 = broadcast1(a.toT1, math.acos)
...
trait Acos1Spec extends UnaryOp1SpecBase { self: Properties with SpecBackend =>
  ...
  override def genArgN1ForSpecBase: Gen[N1] = Gen.oneOf(
    genV1(value = domain),
  ...
  override def genArgNV1ForSpecBase: Gen[N1] = Gen.oneOf(
    genConst1(value = domain),
    genHalf1(),
    genZero1()
  )
