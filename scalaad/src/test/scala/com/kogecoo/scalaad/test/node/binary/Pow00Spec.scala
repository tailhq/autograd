...
import com.kogecoo.scalaad.graph.{N0, Pow00, Var0}
...
object StdPow00Spec extends Properties("Pow00") with Pow00Spec with StdSpecBackend {
  ...
  override def expectApplyOp(a: N0, b: N0): T0 = math.pow(a.toT0, b.toT0)
...
trait Pow00Spec extends BinaryOp00SpecBase { self: Properties with SpecBackend =>
  ...
  override def op(a: N0, b: N0): N0 = Pow00(a, b)

  override def op(a: String, b: String): String = s"pow($a, $b)"

  override def genLeftArgN0ForSpecBase: Gen[N0] = genNonzeroN0(positiveBaseDomain)

  override def genLeftArgNV0ForSpecBase: Gen[N0] = genNonzeroNV0(positiveBaseDomain)

  override def genLeftArgV0ForSpecBase: Gen[Var0] = genV0(positiveBaseDomain)

  override def genRightArgN0ForSpecBase: Gen[N0] = genNonzeroN0(expDomain)

  override def genRightArgNV0ForSpecBase: Gen[N0] = genNonzeroNV0(expDomain)
