...
import com.kogecoo.scalaad.graph.{Asin1, V1, V2, Var1}
...
object StdAsin1Spec extends Properties("Asin1") with Asin1Spec with StdSpecBackend {
  ...
  override def expectApplyOp(a: V1): T1 = broadcast1(a.toT1, math.asin)
...
trait Asin1Spec extends UnaryOp1SpecBase { self: Properties with SpecBackend =>
  ...
  override def genArgN1ForSpecBase: Gen[V1] = Gen.oneOf(genV1(value = domain),
    genConst1(value = domain),
    genHalf1(),
    genZero1()
  )

  override def genArgNV1ForSpecBase: Gen[V1] = Gen.oneOf(genConst1(value = domain),
    genHalf1(),
    genZero1()
  )
  ...
  override def op(a: V1): V1 = Asin1(a)
