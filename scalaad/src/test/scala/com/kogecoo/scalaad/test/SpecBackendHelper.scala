...
object SpecBackendHelper {
  ...
  case class ToT0[T0](toT0: Expr => T0)

  case class ToT1[T1](toT1: Expr => T1)

  case class ToT2[T2](toT2: Expr => T2)


  object Implicits {

    implicit class N0Ops[T0](val self: Expr) extends AnyVal {
    ...
    implicit class N1Ops[T0, T1](val self: Expr) extends AnyVal {
    ...
    implicit class N2Ops[T0, T2](val self: Expr) extends AnyVal {
