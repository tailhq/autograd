...
import scalaad.graph.{Expr, Real}
...
object SpecBackendHelper {
  ...
  case class ToT0[T0](toT0: Expr[Real] => T0)

  case class ToT1[T1](toT1: Expr[Real] => T1)

  case class ToT2[T2](toT2: Expr[Real] => T2)


  object Implicits {

    implicit class N0Ops[T0](val self: Expr[Real]) extends AnyVal {
    ...
    implicit class N1Ops[T0, T1](val self: Expr[Real]) extends AnyVal {
    ...
    implicit class N2Ops[T0, T2](val self: Expr[Real]) extends AnyVal {
