package com.kogecoo.scalaad.impl.nd4j.op
...
class OrOp(x: INDArray, y: INDArray, z: INDArray) extends BaseOp(x, y, z, x.length()) {
  ...
  private[this] def or(a: Boolean, b: Boolean): Double = {
    if (a || b) 1.0 else 0.0
  }
