...
trait Nd4jTensorEval { self: Nd4jValue =>
  ...
  implicit val eval_nd4j_tensor_double: Eval[Expr[Real], T] = new Eval[Expr[Real], T] {
    ...
    def eval(n: Expr[Real]): T = n.shape.order match {
      case s if s > 0 => n match {

        // Nullary op
        case a: Var                    => a.data.value[T]
        case a: Const[Real] @unchecked => a.data.value[T]
        case a: Diag                   => Nd4j.diag(a.diagVec.value[T])
  ...
  implicit val eval_ndj4_tensor_bool: Eval[Expr[Bool], B] = new Eval[Expr[Bool], B] {

    def eval(n: Expr[Bool]): B = n.shape.order match {
      case s if s > 0 => n match {
        ...
        case Not(v)    => v.eval[B].map { a => if (a == 0) 1.0 else 0.0 }
        case And(l, r) => Nd4jUtil.and(l.eval[B], r.eval[B])
        case Or(l, r)  => Nd4jUtil.or(l.eval[B], r.eval[B])
      }
    }
  }
